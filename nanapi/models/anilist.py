...
class ALMedia(ALBaseModel):
    ...
    characters: ALCharacterConnection | None = None
...
class ALMediaEdge(BaseModel):
    ...
    characterRole: Literal['MAIN', 'SUPPORTING', 'BACKGROUND'] | None = None

    def to_edgedb(self):
        sorted_va = sorted(self.voiceActors, key=lambda va: -(va.favourites or 0))
        data = dict(
            media=self.node,
            voice_actors=[va.id for va in sorted_va],
...
class ALCharacter(ALBaseModel):
    ...
    @override
    def to_edgedb(self, include_computed: bool = False, is_updating: bool = True):
        data = super().to_edgedb()
        assert self.media is not None
        last_update = int(time.time()) if is_updating else 0
        data |= dict(
            description=(self.description if self.description else None),
            gender=self.gender,
            age=self.age,
...
_ = ALCharacterConnection.model_rebuild()
_ = ALStaff.model_rebuild()
